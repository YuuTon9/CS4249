...
def get_dog_data(gender, age, limit, hdb_approved):
    if not gender and age == -1 and hdb_approved == "":
        a = Dog_tab.query.limit(limit).all()
    elif not gender and age == -1:
        a = Dog_tab.query.filter_by(hdb_approved=hdb_approved).limit(limit).all()
    elif not gender and hdb_approved == "":
        a = Dog_tab.query.filter_by(age=age).limit(limit).all()
    elif age == -1 and hdb_approved == "":
        a = Dog_tab.query.filter_by(gender=gender).limit(limit).all()
    elif not gender:
        a = Dog_tab.query.filter_by(hdb_approved=hdb_approved).filter_by(age=age).limit(limit).all()
    elif age == -1:
        a = Dog_tab.query.filter_by(hdb_approved=hdb_approved).filter_by(gender=gender).limit(limit).all()
    elif hdb_approved == "":
        a = Dog_tab.query.filter_by(age=age).filter_by(gender=gender).limit(limit).all()
    else:
        a = Dog_tab.query.filter_by(gender=gender).filter_by(age=age).filter_by(hdb_approved=hdb_approved).limit(
        limit).all()
