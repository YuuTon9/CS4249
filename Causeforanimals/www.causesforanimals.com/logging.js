...
// Log the given event.
    export function logEvent(event, customName, customInfo, technique, listLength) {
        ...
        if (ENABLE_CONSOLE_LOGGING) {
            console.log(uid, time, eventName, target, info, state, techniqueMapping[technique], listLength, LOG_VERSION);
        }
        if (ENABLE_NETWORK_LOGGING) {
            sendNetworkLog(uid, time, eventName, target, info, state, techniqueMapping[technique], listLength, LOG_VERSION);
        }
    }
...
function sendNetworkLog(
    ...
    state,
    log_version) {
    var formid = "e/1FAIpQLSeYwnkblRdSaZjRydZcQz2vOa6Okm0tZVCS7YUA8d1iMvseoA";
    var data = {
        "entry.1722620362": uid,
        "entry.796345184": time,
        "entry.1038967963": eventname,
        "entry.1468107578": target,
        "entry.613351431": info,
        "entry.1768464561": state,
        "entry.335400565": log_version
    };
    var params = [];
    var key;
    for (key in data) {
